...
**更新摘要**
本次更新反映了分布式训练清理过程的重要改进：所有训练脚本都增加了分布式训练清理过程中的同步屏障调用，防止分布式训练终止时的挂起问题，显著提升了分布式训练的稳定性。
...
**图表来源**
- [trainer/train_pretrain.py:1-172](file://trainer/train_pretrain.py#L1-172)
- [trainer/train_full_sft.py:1-173](file://trainer/train_full_sft.py#L1-173)
- [trainer/train_lora.py:1-186](file://trainer/train_lora.py#L1-186)
- [trainer/train_dpo.py:1-228](file://trainer/train_dpo.py#L1-228)
- [trainer/train_ppo.py:1-437](file://trainer/train_ppo.py#L1-437)
- [trainer/train_grpo.py:1-334](file://trainer/train_grpo.py#L1-334)
- [trainer/train_distillation.py:1-248](file://trainer/train_distillation.py#L1-248)
- [trainer/train_agent.py:1-492](file://trainer/train_agent.py#L1-492)
...
- **分布式训练清理：所有训练脚本在训练结束后执行同步屏障(dist.barrier())和进程组销毁(dist.destroy_process_group())，防止分布式训练终止时的挂起问题，显著提升分布式训练稳定性。**

**章节来源**
- [trainer/trainer_utils.py:44-117](file://trainer/trainer_utils.py#L44-L117)
- [trainer/train_full_sft.py:109-173](file://trainer/train_full_sft.py#L109-L173)
- [trainer/train_lora.py:103-186](file://trainer/train_lora.py#L103-L186)
- [trainer/train_dpo.py:157-228](file://trainer/train_dpo.py#L157-L228)
- [trainer/train_ppo.py:348-437](file://trainer/train_ppo.py#L348-L437)
- [trainer/train_grpo.py:245-334](file://trainer/train_grpo.py#L245-L334)
- [trainer/train_distillation.py:178-248](file://trainer/train_distillation.py#L178-L248)
- [trainer/train_agent.py:469-492](file://trainer/train_agent.py#L469-L492)
- [trainer/rollout_engine.py:196-213](file://trainer/rollout_engine.py#L196-L213)

## 架构总览
训练流程以"脚本驱动 + 工具函数 + 模型/数据集"为核心，统一通过分布式初始化、混合精度、GradScaler、学习率调度与检查点管理串联起各阶段训练。所有训练脚本在训练完成后都会执行分布式清理过程，确保分布式训练的稳定终止。
...
Script->>Script : 分布式清理(dist.barrier()+destroy_process_group)
```
...
### 分布式训练清理与稳定性增强
**更新** 所有训练脚本都增加了分布式训练清理过程中的同步屏障调用，防止分布式训练终止时的挂起问题，显著提升分布式训练稳定性。

- **同步屏障(dist.barrier)**：在销毁进程组之前，所有分布式进程必须到达同步点，确保所有进程都已完成当前操作。
- **进程组销毁(dist.destroy_process_group)**：安全地清理分布式进程组资源，释放相关系统资源。
- **统一清理模式**：预训练、指令微调、LoRA 微调、DPO、PPO、GRPO、蒸馏等所有训练脚本都遵循相同的清理流程。

```mermaid
flowchart TD
CleanStart(["分布式训练清理"]) --> CheckInit{"dist.is_initialized()?"}
CheckInit --> |否| CleanEnd(["无需清理"])
CheckInit --> |是| Barrier["dist.barrier() 同步屏障"]
Barrier --> Destroy["dist.destroy_process_group() 销毁进程组"]
Destroy --> CleanEnd
```

**图表来源**
- [trainer/train_pretrain.py:169-172](file://trainer/train_pretrain.py#L169-L172)
- [trainer/train_full_sft.py:170-173](file://trainer/train_full_sft.py#L170-L173)
- [trainer/train_lora.py:183-186](file://trainer/train_lora.py#L183-L186)
- [trainer/train_dpo.py:225-228](file://trainer/train_dpo.py#L225-L228)
- [trainer/train_ppo.py:435-437](file://trainer/train_ppo.py#L435-L437)
- [trainer/train_grpo.py:331-334](file://trainer/train_grpo.py#L331-L334)
- [trainer/train_distillation.py:245-248](file://trainer/train_distillation.py#L245-L248)
...
- **分布式清理依赖 torch.distributed 模块提供的同步屏障和进程组管理功能。**
...
- [trainer/train_agent.py:1-492](file://trainer/train_agent.py#L1-L492)
- [trainer/rollout_engine.py:1-213](file://trainer/rollout_engine.py#L1-L213)
- [model/model_lora.py:1-66](file://model/model_lora.py#L1-L66)
- [model/model_minimind.py:1-280](file://model/model_minimind.py#L1-L280)
- [dataset/lm_dataset.py:1-256](file://dataset/lm_dataset.py#L1-L256)
...
**章节来源**
- [trainer/train_full_sft.py:1-173](file://trainer/train_full_sft.py#L1-L173)
- [trainer/train_lora.py:1-186](file://trainer/train_lora.py#L1-L186)
- [trainer/train_dpo.py:1-228](file://trainer/train_dpo.py#L1-L228)
- [trainer/train_ppo.py:1-437](file://trainer/train_ppo.py#L1-L437)
- [trainer/train_grpo.py:1-334](file://trainer/train_grpo.py#L1-L334)
- [trainer/train_distillation.py:1-248](file://trainer/train_distillation.py#L1-L248)
- [trainer/train_pretrain.py:1-172](file://trainer/train_pretrain.py#L1-L172)
- [trainer/train_agent.py:1-492](file://trainer/train_agent.py#L1-L492)
- [trainer/rollout_engine.py:1-213](file://trainer/rollout_engine.py#L1-L213)
- [model/model_lora.py:1-66](file://model/model_lora.py#L1-L66)
- [model/model_minimind.py:1-280](file://model/model_minimind.py#L1-L280)
...
- **分布式清理优化**：同步屏障确保所有进程协调退出，避免资源泄漏和挂起问题。
...
- **分布式清理问题**
  - **同步屏障阻塞**：如果训练结束后出现挂起，检查所有进程是否都到达了 dist.barrier() 调用点。
  - **进程组销毁失败**：确保在调用 dist.destroy_process_group() 之前，所有分布式操作都已完成。
  - **资源泄漏**：定期检查系统进程，确认分布式训练结束后没有残留的进程组。
- 检查点恢复异常
...
## 结论
MiniMind 的训练工具与实用程序以"可复现、可扩展、可移植"为目标，通过统一的分布式初始化、随机种子、混合精度、检查点管理与学习率调度，支撑从预训练到指令微调、LoRA、DPO、PPO/GRPO、蒸馏等全链路训练。**最新的分布式训练清理优化显著提升了训练稳定性，通过同步屏障和进程组销毁确保分布式训练的可靠终止。**配套的 rollout 引擎与数据集实现进一步提升了训练灵活性与效率。建议在生产环境中结合 SwanLab 可视化、分布式训练与检查点恢复机制，确保训练稳定性与可追溯性。

## 附录
- 训练命令与参数说明可参考 README 的"快速开始"与"主要训练"章节。
- 各训练脚本均支持 --from_resume 自动检测与恢复训练进度。
- 可视化工具默认使用 SwanLab（兼容 W&B），便于国内网络环境使用。
- **分布式训练清理流程已在所有训练脚本中标准化实施，确保训练过程的稳定性和可靠性。**
