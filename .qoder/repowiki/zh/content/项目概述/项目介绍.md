...
- **多框架兼容生态**：支持Transformers、llama.cpp、vllm、ollama等主流推理框架，以及OpenAI API协议
...
- **工具调用**：内置8种工具的可视化选择，支持多轮Tool Call交互
- **自适应思考**：通过模板层实现思考能力的动态开关控制
...
I --> J[LoRA模块]
K[AgentRLDataset] --> L[train_agent.py]
L --> M[RolloutEngine]
end
subgraph "推理系统"
N[eval_llm.py] --> O[AutoTokenizer]
N --> P[TextStreamer]
Q[web_demo.py] --> R[Streamlit UI]
S[serve_openai_api.py] --> T[FastAPI服务]
U[chat_api.py] --> V[OpenAI SDK]
W[convert_model.py] --> X[模型转换]
end
subgraph "数据处理"
Y[lm_dataset.py] --> Z[JSONL数据]
Z --> AA[Chat模板]
AA --> BB[Tokenization]
end
A --> I
...
- [train_lora.py:1-186](file://trainer/train_lora.py#L1-L186)
- [lm_dataset.py:1-256](file://dataset/lm_dataset.py#L1-L256)
- [train_agent.py:1-200](file://trainer/train_agent.py#L1-L200)
...
6. **Rollout引擎**：支持PyTorch原生和SGLang的多引擎推理
7. **OpenAI兼容**：提供完整的OpenAI API协议支持
...
1. **多格式支持**：统一处理预训练、SFT、DPO、RLAIF、Agent RL等多种数据格式
2. **模板系统**：支持复杂的对话模板，包括工具调用和思考标签
3. **动态处理**：根据配置动态启用不同的数据处理策略
4. **工具集成**：内置8种工具的模拟执行，支持多轮Tool Call交互
...
**更新** LoRA训练脚本的正确执行方式已明确，需要在trainer目录下执行：
...
**图表来源**
- [train_lora.py:78-186](file://trainer/train_lora.py#L78-L186)

### 强化学习训练流程

```mermaid
flowchart TD
A[训练脚本] --> B[Rollout Engine]
B --> C[多轮对话生成]
C --> D[工具调用执行]
D --> E[奖励计算]
E --> F[策略更新]
...
**图表来源**
- [train_agent.py:98-180](file://trainer/train_agent.py#L98-L180)
- [rollout_engine.py:71-93](file://trainer/rollout_engine.py#L71-L93)
...
G[聊天API] --> H[chat_api.py]
I[模型转换] --> J[convert_model.py]
end
subgraph "兼容生态"
K[transformers]
L[llama.cpp]
...
1. **完整协议支持**：支持reasoning_content、tool_calls、open_thinking参数
2. **流式响应**：支持SSE流式输出
3. **工具调用**：原生支持OpenAI风格的工具调用格式
4. **思考内容**：支持思考过程的流式传输
...
| MiniMind-3-MoE | 198M-A64M | ≈1.69小时 | ≈2.20元 |
