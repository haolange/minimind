...
- **分布式训练清理改进** [新增]
  - 在训练结束时统一调用 dist.barrier() 确保所有进程同步，然后销毁进程组，避免分布式训练终止时的挂起问题。
...
- [trainer/train_pretrain.py:170-172](file://trainer/train_pretrain.py#L170-L172)
- [trainer/train_full_sft.py:171-173](file://trainer/train_full_sft.py#L171-L173)
- [trainer/train_distillation.py:246-248](file://trainer/train_distillation.py#L246-L248)
- [trainer/train_lora.py:184-186](file://trainer/train_lora.py#L184-L186)
- [trainer/train_dpo.py:226-228](file://trainer/train_dpo.py#L226-L228)
- [trainer/train_agent.py:490-492](file://trainer/train_agent.py#L490-L492)

## 架构总览
下图展示典型训练脚本的 DDP 流程：初始化 DDP → 构造模型与优化器 → 包装 DDP → 分布式采样 → 前向/反向/优化 → 保存检查点 → **统一清理分布式进程**（新增 barrier 同步）。
...
### 分布式训练清理流程 [更新]
- 统一清理流程：在训练结束时，所有训练脚本都调用 dist.barrier() 确保进程同步，然后销毁进程组。
- 解决挂起问题：新增的 dist.barrier() 调用解决了分布式训练终止时的进程挂起问题，确保所有进程都能正常退出。
- 一致性保证：通过 barrier 同步，避免部分进程提前退出而其他进程仍在等待的情况。

```mermaid
flowchart TD
Start(["训练结束"]) --> CheckInit{"dist.is_initialized()?"}
CheckInit --> |是| Barrier["dist.barrier() 同步所有进程"]
CheckInit --> |否| End(["直接结束"])
Barrier --> Destroy["dist.destroy_process_group()"]
Destroy --> End
```
...
- **清理流程依赖**：所有训练脚本都依赖统一的分布式清理流程，确保进程组正确销毁。
...
- **清理流程优化**：新增的 dist.barrier() 调用不会显著影响训练性能，但能确保进程组正确销毁，避免资源泄漏。
...
- 排查要点
  ...
  - **检查清理流程**：确认 dist.barrier() 是否在 destroy_process_group() 前调用。
- 处理建议
  - 在 DDP 包装后显式忽略缓冲；核对 dtype 与 scaler 使用。
  - 减少 num_workers 或禁用 pin_memory，排查数据加载竞争。
  - 降低 batch size 或增大 accumulation_steps，缓解显存压力。
  - **确保清理顺序正确**：先调用 dist.barrier() 同步，再调用 dist.destroy_process_group() 销毁进程组。
...
- 错误传播
  ...
  - **清理流程监控**：通过观察训练结束后的进程退出情况，判断清理流程是否正常执行。
...
- 节点故障与恢复
  ...
  - **清理流程鲁棒性**：新增的 dist.barrier() 调用提高了分布式训练清理的可靠性，减少因清理不当导致的节点故障。
...
### 分布式训练清理问题 [新增]
- 现象
  - 训练结束后进程无法正常退出，出现挂起状态。
  - 部分进程提前退出，其他进程仍在等待。
- 排查要点
  - 检查训练脚本末尾是否调用了 dist.barrier()。
  - 确认 dist.destroy_process_group() 是否在 dist.barrier() 之后调用。
  - 观察所有进程是否在同一时间到达清理点。
- 处理建议
  - 确保所有训练脚本都包含完整的清理流程：dist.barrier() → dist.destroy_process_group()。
  - 如果某个进程因为异常提前退出，其他进程会在 barrier 处等待超时。
  - 使用超时参数设置合理的等待时间，避免无限期等待。
...
## 结论
MiniMind 的分布式训练以 DDP 为核心，通过统一的工具函数实现初始化、主进程判定、断点续训与混合精度管理。针对多 GPU 训练中的常见问题，建议从进程同步、NCCL 通信、内存分配与负载均衡四个维度入手排查，并结合断点续训与日志策略提升稳定性与可恢复性。**最新的分布式训练清理改进**通过在训练结束时统一调用 dist.barrier()，有效解决了分布式训练终止时的挂起问题，提高了训练过程的可靠性和进程组销毁的正确性。在不同集群环境下，重点保障 NCCL 环境与网络连通，配合合理的 batch size/accumulation_steps 与 max_seq_len，可有效降低异常发生概率。
...
- **清理流程清单** [更新]
  - 所有训练脚本均已实现统一的分布式清理流程，确保训练结束后进程能够正确退出。
